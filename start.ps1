...
if (-not (Test-Path $python)) {
    ...
    Write-Host "Run:  python -m venv .venv  then  .venv\Scripts\pip install -r backend\requirements.txt" -ForegroundColor Yellow
    exit 1
}
...
Start-Process powershell -ArgumentList `
    "-NoExit", `
    "-Command", `
    "Write-Host 'Flask API' -ForegroundColor Cyan; Set-Location '$root'; & '$python' '$api'"
